...
$query = "SELECT ParentID,CollectionID,COUNT(*) AS C FROM tblCollections_Content WHERE CollectionID !=0 AND SortOrder = 1
   GROUP BY CollectionID,ParentID HAVING C>1";
...
$query = "SELECT ParentID,CollectionID,COUNT(DISTINCT(LevelContainerID)) AS C FROM tblCollections_Content WHERE CollectionID != 0
GROUP BY CollectionID,ParentID HAVING C>1";
