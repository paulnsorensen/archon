...
if (!empty($objDigitalContent->Files))
{
   ...
   foreach ($objDigitalContent->Files as $objFile)
   {
      ...
      if ($_ARCHON->Security->verifyPermissions(MODULE_DIGITALLIBRARY, READ) && $objFile->DefaultAccessLevel != DIGITALLIBRARY_ACCESSLEVEL_FULL)
      {
         ...
         {
            echo("<div class='digcontenttitlebox mdround'>");
            echo("<span class='bold'>NOTE</span>:  This file is enabled for previews only.<br/>");
            echo("</div>");
         }
         elseif ($objFile->DefaultAccessLevel == DIGITALLIBRARY_ACCESSLEVEL_NONE)
         {
            echo("<div class='digcontenttitlebox mdround'>");
            echo("<span class='bold'>NOTE</span>: This file is <span class='bold'>NOT</span> enabled for previews or downloading.<br/>");
            echo("</div>");
         }
      }
      ...
      else
      {
         if (encoding_substr_count($objFile->FileType->FileExtensions, '.pdf') && file_exists("{$_ARCHON->PublicInterface->ImagePath}/pdficon_large.gif"))
         {
            ...
            echo("<span class='digcontentfiletitle'>" . ($objFile->getString('Title')) . "<br/><a href='?p=digitallibrary/getfile&amp;id=$objFile->ID' $onclick \">Download</a> (" . $objFile->FileType->getString('FileType') . ", " . formatsize($objFile->Size) . ")</span><br/><br/>");
         }
         elseif ($objFile->FileType->MediaType->MediaType == 'Image')
         {
            ...
            echo("<a href='index.php?p=digitallibrary/getfile&amp;id=$objFile->ID' $onclick ><img class='digcontentfile' src='".$objFile->getFileURL(DIGITALLIBRARY_FILE_PREVIEWLONG)."' alt='{$objFile->getString('Title')}'/></a>");
         }
